                ...
                <div class="card rounded-0 border-0 mb-3">
                    <div class="card-header">
                        <div class="row">
                            ...
                            <div class="col-5">
                            {!! Form::open(['url' =>'printNewCandidateListByUdc','method' => 'post','role' => 'form','class'=>'form-horizontal']) !!}
                            ...
                            <tbody>
                                <?php $j = 1 ; foreach ($result as $value) { ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php if($value->berth_reg_complete == "1") : ?>
                                                <a href="{{URL::to('updateCandiateBirthReg/'.$value->id)}}" title="">
                                                    <button type="button" class="btn btn-warning btn-sm">Update Birth Reg</button>
                                                </a>
                                            <?php elseif($value->berth_reg_complete == 0 AND $value->berth_reg_distribution == 1) : ?>
                                                <a href="{{URL::to('candidateBirthRegDustributionComplete/'.$value->id)}}">
                                                    <button type="button" class="btn btn-danger btn-sm" onclick="return confirm('Are You Sure You Want To Confrim ?')">Distribution Compete</button>
                                                </a>
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <a href="{{URL::to('viewNewCandidatesByUdc/'.$value->id)}}" title="">
                                                <button type="button" class="btn btn-success btn-sm">View</button>
                                            </a> 
                                            <a href="{{URL::to('editCandidatesByUdc/'.$value->id)}}" title="">
